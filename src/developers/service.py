    ...
    def find_all(self, query: DeveloperQuerySchema):
        ...
        base = (
            self.db.query(
                ...
                func.round(func.avg(GameRelease.meta_score).cast_type(None), 2).label("avg_meta_score"),
                func.round(func.avg(GameRelease.user_review).cast_type(None), 2).label("avg_user_review"),
                func.round(func.sum(GameRelease.total_sales).cast_type(None), 2).label("total_sales"),
            )
